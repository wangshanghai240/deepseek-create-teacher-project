      ...
      >
        ...
        <div class="article-card-body">
          ...
          <div class="article-meta">
            <span class="article-source">📰 央视新闻</span>
...
export default {
  ...
  setup() {
    ...
    const categories = ref([
      ...
      { id: 'china', name: '国内', icon: '🇨🇳' },
      { id: 'world', name: '国际', icon: '🌍' },
      { id: 'photo', name: '图片', icon: '📷' },
      { id: 'people', name: '人物', icon: '👤' },
      { id: 'mil', name: '军事', icon: '⚔️' },
    ])
    ...
    const fetchArticles = async () => {
      ...
      error.value = ''
      try {
        const res = await fetch('/api/cctv/list?cat=' + activeCategory.value + '&page=1&pageSize=10', { timeout: 15000 })
    ...
    const loadMore = async () => {
      ...
      try {
        ...
        const res = await fetch('/api/cctv/list?cat=' + activeCategory.value + '&page=' + nextPage + '&pageSize=10', { timeout: 15000 })
        const result = await res.json()
    ...
    return { categories, activeCategory, articles, loading, loadingMore, error, switchCategory, fetchArticles, loadMore, openArticle }
  }
}
</script>
