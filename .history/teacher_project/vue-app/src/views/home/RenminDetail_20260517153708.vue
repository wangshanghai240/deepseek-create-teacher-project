  ...
  setup() {
    ...
    onMounted(async () => {
      ...
      if (!article.value?.content) {
        ...
        try {
          const apiUrl = '/api/cctv/detail?url=' + encodeURIComponent(article.value.url)
...
.article-content :deep(.article-video-placeholder) {
  margin: 16px 0;
  padding: 32px 20px;
...
.article-content :deep(.video-play-icon) {
  font-size: 48px;
  color: #fff;
  margin-bottom: 12px;
  opacity: 0.9;
...
.article-content :deep(.video-hint) {
  color: #ccc;
  font-size: 15px;
  margin-bottom: 8px;
...
.article-content :deep(.video-hint-small) {
  color: #888;
  font-size: 11px;
  margin-bottom: 16px;
  text-indent: 0 !important;
...
.article-content :deep(.video-link) {
...
.article-content :deep(.video-link:hover) {
  transform: translateY(-1px);
  box-shadow: 0 4px 14px rgba(129, 140, 248, 0.4);
}

.article-content :deep(.logo-gallery) {
  margin-top: 20px;
  padding: 12px;
  background: var(--bg-hover);
  border-radius: 10px;
...
.article-content :deep(.logo-label) {
  font-size: 11px;
  color: var(--text-muted);
  display: block;
  margin-bottom: 8px;
...
.article-content :deep(.logo-list) {
  display: flex;
  gap: 8px;
  flex-wrap: wrap;
}

.article-content :deep(.logo-thumb) {
  width: 40px;
  height: 40px;
  object-fit: contain;
  border-radius: 6px;
