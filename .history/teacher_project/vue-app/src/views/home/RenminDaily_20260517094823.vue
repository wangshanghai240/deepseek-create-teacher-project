      ...
      >
        ...
        <div class="article-card-body">
          ...
          <div class="article-meta">
            ...
            <span v-if="hasArticleUrl(item)" class="detail-badge">查看内容</span>
            <span v-else class="external-badge">阅读原文 ↗</span>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  setup() {
    ...
    const hasArticleUrl = (item) => {
      return item.url && item.url.includes('/article/')
    }
    ...
    const openArticle = (item) => {
      // 有真实文章链接的进入详情页，否则直接打开浏览器
      if (hasArticleUrl(item)) {
