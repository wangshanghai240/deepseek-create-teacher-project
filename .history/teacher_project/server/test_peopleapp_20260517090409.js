...
async function test() {
  ...
  // 从配置中提取 API 地址
  const configMatch = html.match(/VITE_API_RMRB_BFF_DISPLAY[^,]+/);
  if (configMatch) console.log('API Config:', configMatch[0]);
  
  // 提取 __NUXT_DATA__ 的数据
  const match = html.match(/<script id="__NUXT_DATA__"[^>]*type="application\/json"[^>]*>([\s\S]*?)<\/script>/);
  if (match) {
