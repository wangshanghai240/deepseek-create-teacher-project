...
router.get('/cctv/detail', async (req, res) => {
  try {
    ...
    // 从JS变量 contentdate 获取完整HTML内容（含可能嵌入的视频）
    const contentMatch = r.data.match(/var\s+contentdate\s*=\s*'([\s\S]*?)';/);
    let rawContentHtml = '';
    if (contentMatch) {
      rawContentHtml = contentMatch[1]
        .replace(/\\n/g, '\n').replace(/\\'/g, "'").replace(/\\"/g, '"');
    }
    ...
    if (rawContentHtml) {
      // 去除 htmlVideoCode 标记
      const cleanHtml = rawContentHtml.replace(/\[!--begin:htmlVideoCode--\][a-f0-9,:\-newPlayer]+\[!--end:htmlVideoCode--\]/g, '');
      const text = cleanHtml.replace(/<[^>]+>/g, '').replace(/&nbsp;/g, ' ').replace(/\s+/g, ' ').trim();
    ...
    // 提取视频 - 从多种格式
    const videos = [];
    const videoIds = []; // CCTV htmlVideoCode IDs
    ...
    // 从 contentdate HTML 中提取（CCTV文章正文可能嵌入视频）
    if (rawContentHtml) {
      // 提取 htmlVideoCode 视频ID
      const hvcRegex = /htmlVideoCode--\]([a-f0-9]+),/gi;
      let hvcMatch;
      while ((hvcMatch = hvcRegex.exec(rawContentHtml)) !== null) {
        if (!videoIds.includes(hvcMatch[1])) videoIds.push(hvcMatch[1]);
      }
      // 从视频ID构造可用的播放页URL
      for (const vid of videoIds) {
        videos.push('https://news.cctv.com/player/player.html?guid=' + vid);
      }
      
      const vids = rawContentHtml.match(/https?:[^"'\s]*\.(mp4|m3u8|flv)[^"'\s]*/gi);
      if (vids) vids.forEach(v => { if (!videos.includes(v)) videos.push(v); });
      const iframes = rawContentHtml.match(/<iframe[^>]*src=["']([^"']+)["'][^>]*>/gi);
      if (iframes) iframes.forEach(f => { const s = (f.match(/src=["']([^"']+)["']/)||[])[1]; if (s && !videos.includes(s)) videos.push(s); });
    }
    ...
    if (hvcMatch) {
      ...
      if (iframeSrc && !videos.includes(iframeSrc[1])) videos.push(iframeSrc[1]);
    ...
    if (hasVideo) {
      ...
      if (firstVideo.includes('//') && !firstVideo.match(/\.(mp4|m3u8|flv)$/i)) {
        // CCTV播放器需要JS才能加载，添加视频播放提示
        const videoId = videoIds[0] || '';
        parts.push('<div class="article-video-placeholder">');
        parts.push('<div class="video-play-icon">▶</div>');
        parts.push('<p class="video-hint">该文章包含视频内容</p>');
        if (videoId) parts.push('<p class="video-hint-small">视频ID: ' + videoId + '</p>');
        parts.push('<a class="video-link" href="' + url + '" target="_blank" rel="noopener noreferrer">在央视网观看完整视频 ↗</a>');
        parts.push('</div>');
      } else {
