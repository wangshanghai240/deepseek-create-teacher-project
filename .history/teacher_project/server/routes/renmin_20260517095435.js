...
function extractArticlesFromHtml(html) {
  ...
  try {
    ...
    function addArticle(item, timeIdx) {
      ...
      let image = '';
      const imgIdx = item.image || item.image_url;
      ...
      // 检测是否有视频/音频
      const hasVideo = !!(item.video_url || item.video || item.videoUrl);
      const hasVoice = !!item.voiceInfo;
      ...
      articles.push({
        ...
        source: '人民日报',
        url: articleUrl,
        hasVideo: hasVideo || hasVoice
      });
    }

    let tsIdx = 0;

    // 方法1: 从主 feed 列表中提取（title + relId）
    for (let i = 0; i < rawData.length; i++) {
      const item = rawData[i];
      if (item && typeof item === 'object' && !Array.isArray(item)) {
        const keys = Object.keys(item);
        if (keys.includes('title') && keys.includes('relId')) {
          addArticle(item, timestamps[tsIdx]);
          tsIdx++;
        }
      }
    }
    ...
    // 方法2: 从 hot 热点板块提取
    // hot 对象有 list 数组，包含更多文章索引
    for (let i = 0; i < rawData.length; i++) {
      const item = rawData[i];
      if (item && typeof item === 'object' && !Array.isArray(item) && item.list) {
        const listIdx = item.list;
        if (typeof listIdx === 'number') {
          const listArr = rawData[listIdx];
          if (Array.isArray(listArr)) {
            for (const idx of listArr) {
              if (typeof idx === 'number') {
                const articleObj = rawData[idx];
                if (articleObj && typeof articleObj === 'object' && !Array.isArray(articleObj)) {
