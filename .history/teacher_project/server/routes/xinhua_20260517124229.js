...
async function fetchTodayArticles() {
  ...
  const urls = new Set();
  // 匹配所有 news.cn 的文章链接
  // URL格式: https://www.news.cn/.../20260517/uuid/c 或 /c.html
  // 注意HTML中URL是连在一起的: .../chttps://...，需用选择器匹配
  const linkRegex = /<a[^>]*href="(\/[^"]*\/2026\d{4}\/[a-f0-9]+\/c[^"]*)"[^>]*>/gi;
  let m;
  while ((m = linkRegex.exec(html)) !== null) {
    const href = m[1].replace(/\..$/, ''); // 处理可能截断的href
    const url = 'https://www.news.cn' + href;
    if (url.match(/\/c(\.html)?$/)) urls.add(url);
  }
