...
router.get('/cctv/detail', async (req, res) => {
  try {
    ...
    if (rawContentHtml) {
      ...
      // 提取 CCTV 特有的 htmlVideoCode 格式: [!--begin:htmlVideoCode--]videoId,0,1,16:9,newPlayer[!--end:htmlVideoCode--]
      const hvcMatches = rawContentHtml.match(/htmlVideoCode--\]([a-f0-9]+),/gi);
      if (hvcMatches) {
        hvcMatches.forEach(m => {
          const vid = (m.match(/htmlVideoCode--\]([a-f0-9]+),/)||[])[1];
          if (vid) videos.push('https://news.cctv.com/player/video.php?guid=' + vid);
        });
      }
    }
