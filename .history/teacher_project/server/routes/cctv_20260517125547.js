...
router.get('/cctv/detail', async (req, res) => {
  try {
    ...
    $('img[src]').each((i, el) => {
      let s = $(el).attr('src');
      if (s && s.match(/\.(jpg|jpeg|png)/i) && !s.includes('.ico')) {
