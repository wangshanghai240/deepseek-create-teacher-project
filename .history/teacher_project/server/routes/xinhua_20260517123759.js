...
async function fetchTodayArticles() {
  ...
  const html = res.data;
  const urls = new Set();
  // 匹配所有 news.cn 文章 URL（用前瞻避免连在一起的URL）
  const pattern = /https?:\/\/[^"'\s<>]*news\.cn[^"'\s<>]*?\/2026\d{4}\/[a-f0-9]+\/c(?=\.html|["'\s<>]|https?:\/\/|$)/g;
  const rawMatches = html.match(pattern) || [];
  for (let url of rawMatches) {
    url = url.replace(/["'<>]+/g, '');
    if (url && url.match(/\/c$/)) urls.add(url);
  }
