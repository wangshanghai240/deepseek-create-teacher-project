...
function extractArticles(html, baseUrl) {
  ...
  // 匹配文章链接: /category/YYYYMMDD/uuid/c[.html]
  const linkRegex = /<a[^>]*href="([^"]*\/20\d{6}\/[a-f0-9]+\/[^"]*)"[^>]*>([\s\S]*?)<\/a>/gi;
  let match;
  while ((match = linkRegex.exec(html)) !== null) {
    ...
    if (!url.startsWith('http')) {
      const base = baseUrl || 'https://www.news.cn';
    ...
    if (image && !image.startsWith('http')) {
      image = 'https://www.news.cn' + image;
    }
    
    if (title && title.length > 5 && !seen.has(title) && !title.includes('javascript')) {
      seen.add(title);
      articles.push({
