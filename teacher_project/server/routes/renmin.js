...
function extractContentFromHtml(html, targetRelId) {
  ...
  try {
    ...
    if (targetItem) {
      for (const [key, val] of Object.entries(targetItem)) {
        if ((key === 'newsTxt' || key === 'newsSummary') && typeof val === 'number' && strings[val]) {
          const t = strings[val].trim();
          if (t.length > 20) targetParagraphs.push(t);
        }
    ...
    for (let i = 0; i < rawData.length; i++) {
      const item = rawData[i];
      if (item && typeof item === 'object' && !Array.isArray(item) && item !== targetItem) {
        for (const [key, val] of Object.entries(item)) {
          if ((key === 'newsTxt' || key === 'newsSummary') && typeof val === 'number' && strings[val]) {
            const t = strings[val].trim();
            if (t.length > 20) otherParagraphs.push(t);
          }
        }
      }
    }

    // 合并：目标文章内容在前，补充内容在后（去重）
    // 如果目标文章没有文字内容（如热点文章），则不添加其他文章的文字避免不对应
    const combined = [...targetParagraphs];
    if (targetParagraphs.length > 0) {
      const targetSet = new Set(targetParagraphs);
      for (const p of otherParagraphs) {
        if (!targetSet.has(p) && combined.length < 8) combined.push(p);
      }
    }
    ...
    for (let pi = 0; pi < combined.length; pi++) {
      const cleanP = combined[pi].replace(/<[^>]+>/g, '').replace(/&nbsp;/g, ' ').replace(/\s+/g, ' ').trim();
      if (cleanP.length > 10) {
